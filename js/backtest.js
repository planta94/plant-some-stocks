...
async function runBacktestSimulation() {
  ...
  try {
    ...
    clearInterval(btProgressInterval);
    console.warn("Backtest API unavailable, attempting static cache fallback...", err);
    if (window.STATIC_BACKTEST_DATA) {
      completeProgressMonitor("Loaded pre-computed backtest simulation data", 100);
      cachedBacktestData = window.STATIC_BACKTEST_DATA;
      renderBacktestDashboard(window.STATIC_BACKTEST_DATA);
    } else {
      document.getElementById("progress-card").classList.add("hidden");
      if (btReturn) btReturn.innerText = "Error";
      console.error("Backtest simulation failed without static fallback", err);
    }
  }
}
...
function renderBacktestDashboard(data) {
  ...
  if (btReturn) btReturn.innerText = data.summary_metrics["Cumulative Return"] || "0.0%";
  if (btWinrate) btWinrate.innerText = data.summary_metrics["Win Rate"] || "0.0%";
  if (btPF) btPF.innerText = data.summary_metrics["Profit Factor"] || "0.0";
  if (btTrades) btTrades.innerText = data.summary_metrics["Total Trades Executed"] || "0";

  // Render Charts
  if (data.equity_curve && data.equity_curve.length > 0) {
    renderEquityCurveChart(data.equity_curve);
  } else if (data.recent_trades && data.recent_trades.length > 0) {
    // Generate equity curve from trades if missing
    let cum = 0;
